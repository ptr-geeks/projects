...
1
// After starting song playback in play_random_song function...

    if let Some(lyrics) = fetch_lyrics_from_flac(&paths[randint]) {
        let parsed_lyrics = parse_lyrics(&lyrics);
        ...
        while !sink.empty() == true {
            ...
            if lyrics_index < parsed_lyrics.len() && elapsed >= parsed_lyrics[lyrics_index].0 {
                println!("{} \r", parsed_lyrics[lyrics_index].1);
                lyrics_index += 1;
            }

            // ... rest of your song loop code ...
        }
    } else {
...
fn parse_lyrics(lyrics: &str) -> Vec<(f32, String)> {
    lyrics.lines()
        .filter_map(|line| {
            let parts: Vec<&str> = line.splitn(2, ']').collect();
            if parts.len() == 2 {
                let time = parts[0].trim_matches('[');
                let (minutes, seconds): (f32, f32) = {
                    let parts: Vec<&str> = time.split(':').collect();
                    (parts[0].parse().unwrap_or(0.0), parts[1].parse().unwrap_or(0.0))
                };
                let total_seconds = minutes * 60.0 + seconds;
                Some((total_seconds, parts[1].to_string()))
            } else {
                None
            }
        })
...
fn fetch_lyrics_from_flac(file_path: &Path) -> Option<String> {
    let output = Command::new("metaflac")
        .arg("--show-tag=LYRICS")
        .arg(file_path)
        .output()
        .expect("Failed to execute metaflac");

    let lyrics = String::from_utf8_lossy(&output.stdout);
    if lyrics.starts_with("LYRICS=") {
        Some(lyrics["LYRICS=".len()..].to_string())
    } else if lyrics.starts_with("lyrics=") {
        Some(lyrics["lyrics=".len()..].to_string())
    } else {
        None
    }
