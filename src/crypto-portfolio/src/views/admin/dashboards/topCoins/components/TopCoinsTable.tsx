/* eslint-disable */ 
import { Badge, Button, Flex, Table, Tbody, Td, Text, Th, Thead, Tr, useColorModeValue, Icon, Stack, Image, Link} from '@chakra-ui/react'; 
...
import { createLinkForCoin } from 'helpers/formatters';
...
export default function TopCoinsTable(props: { tableData: RowObjCoins[] }) {
  ...
  const [globalFilter, setGlobalFilter] = React.useState('')
    const columnHelper = createColumnHelper<RowObjCoins>();
 	const columns = [
		...
		columnHelper.accessor('name', {
			...
			cell: (info) => ( 
        <Flex>
          <Image src={info.getValue()[2]} width="30px" height="30px" mr="4" rounded={"full"}  />
		...
		columnHelper.accessor('changeHour', {
			id: 'changeHour',
			header: () => (
				<Text
					justifyContent='space-between'
    ...
    columnHelper.accessor('changeWeek', {
			id: 'changeWeek',
			header: () => (
				<Text
					justifyContent='space-between'
    ...
    columnHelper.accessor('marketCap', {
		...
		columnHelper.accessor('id', {
          ...
          MORE
			...
			cell: (info) => ( 
        <Link href={`/#${createLinkForCoin(info.getValue())}`}>
          <Text
              cursor='pointer'
              color={brandColor}
